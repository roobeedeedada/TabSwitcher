...
(function($) {  
	...
	KISCODE.TabSwitcher = function( config ) {
		...
		this.eventType 	= $.inArray( config.eventType, [ 'mouseover', 'click' ] ) > -1 ? config.eventType : 'click';
		this.stateful 	= config.allowCookies;
		this.name 		= config.name;
		this.kiscookie 	= config.name + '-kistabs';
		
		// Check if the jQuery.cookie is available
		if( this.stateful && this.name && $.cookie && $.cookie( this.kiscookie ) ) {
